...
class WPAPP_Comment {
  ...
  function WPAPP_Comment($wp_comment = null) {
    if ($wp_comment) {
  ...
  function import_wp_object($wp_comment) {
    ...
    if (!empty($wp_comment->user_id)) {
      $this->author = new WPAPP_Author($wp_comment->user_id);
    } else {
      unset($this->author);
    }
  }
  ...
  function comment_id_not_found() {
    global $wpapp;
    $wpapp->error("Post ID '{$_REQUEST['post_id']}' not found.");
  }
  
  function comment_closed() {
    global $wpapp;
    $wpapp->error("Post is closed for comments.");
  }
  
  function comment_on_draft() {
    global $wpapp;
    $wpapp->error("You cannot comment on unpublished posts.");
  }
  
  function comment_post_redirect() {
    global $comment, $wpapp;
    $status = ($comment->comment_approved) ? 'ok' : 'pending';
    $new_comment = new WPAPP_Comment($comment);
    $wpapp->response->respond($new_comment, $status);
  }
