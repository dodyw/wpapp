...
class WPAPP_Post {
  ...
  // Note:
  //   WPAPP_Post objects must be instantiated within The Loop.
  ...
  function WPAPP_Post($wp_post = null) {
    if (!empty($wp_post)) {
  ...
  function save($values = null) {
    ...
    if (!empty($values['author'])) {
      $author = $wpapp->introspector->get_author_by_login($values['author']);
    ...
    if (isset($values['categories'])) {
      ...
      foreach ($categories as $category_slug) {
        ...
        $category = $wpapp->introspector->get_category_by_slug($category_slug);
    ...
    if (!empty($_FILES['attachment'])) {
      ...
      $this->attachments[] = new WPAPP_Attachment($attachment_id);
  ...
  function import_wp_object($wp_post) {
    global $wpapp, $post;
    $date_format = $wpapp->query->date_format;
  ...
  function set_content_value() {
    global $wpapp;
    if ($wpapp->include_value('content')) {

      global $more;    // Declare global $more (before the loop).
      $more = 1;

      $content = get_the_content($wpapp->query->read_more);
  ...
  function set_categories_value() {
    global $wpapp;
    if ($wpapp->include_value('categories')) {
      $this->categories = array();
      if ($wp_categories = get_the_category($this->id)) {
        foreach ($wp_categories as $wp_category) {
          $category = new WPAPP_Category($wp_category);
  ...
  function set_tags_value() {
    global $wpapp;
    if ($wpapp->include_value('tags')) {
      $this->tags = array();
      if ($wp_tags = get_the_tags($this->id)) {
        foreach ($wp_tags as $wp_tag) {
          $this->tags[] = new WPAPP_Tag($wp_tag);
        }
      }
    } else {
      unset($this->tags);
  ...
  function set_author_value($author_id) {
    global $wpapp;
    if ($wpapp->include_value('author')) {
      $this->author = new WPAPP_Author($author_id);
    } else {
      unset($this->author);
    }
  }
  ...
  function set_comments_value() {
    global $wpapp;
    if ($wpapp->include_value('comments')) {
      $this->comments = $wpapp->introspector->get_comments($this->id);
    } else {
      unset($this->comments);
    }
  }
  ...
  function set_attachments_value() {
    global $wpapp;
    if ($wpapp->include_value('attachments')) {
      $this->attachments = $wpapp->introspector->get_attachments($this->id);
    } else {
  ...
  function set_thumbnail_value() {
    global $wpapp;
    if (!$wpapp->include_value('thumbnail') ||
  ...
  function set_custom_fields_value() {
    global $wpapp;
    if ($wpapp->include_value('custom_fields') &&
        $wpapp->query->custom_fields) {
      $keys = explode(',', $wpapp->query->custom_fields);
  ...
  function get_thumbnail_size() {
    global $wpapp;
    if ($wpapp->query->thumbnail_size) {
      return $wpapp->query->thumbnail_size;
