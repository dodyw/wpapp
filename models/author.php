...
class WPAPP_Author {
  ...
  // Note:
  //   WPAPP_Author objects can include additional values by using the
  //   author_meta query var.
  
  function WPAPP_Author($id = null) {
    if ($id) {
      $this->id = (int) $id;
    } else {
  ...
  function set_author_meta() {
    ...
    $vars = explode(',', $wpapp->query->author_meta);
