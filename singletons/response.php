...
class WPAPP_Response {
  
  function setup() {
    global $wpapp;
    $this->include_values = array();
    $this->exclude_values = array();
    if ($wpapp->query->include) {
      $this->include_values = explode(',', $wpapp->query->include);
    }
    // Props to ikesyo for submitting a fix!
    if ($wpapp->query->exclude) {
      $this->exclude_values = explode(',', $wpapp->query->exclude);
  ...
  function respond($result, $status = 'ok') {
    ...
    if ($wpapp->query->dev || !empty($_REQUEST['dev'])) {
    ...
    } else if ($wpapp->query->redirect) {
      $url = $this->add_status_query_var($wpapp->query->redirect, $status);
      wp_redirect($url);
    } else if ($wpapp->query->callback) {
      // Run a JSONP-style callback with the result
      $this->callback($wpapp->query->callback, $json);
    } else {
      // Output the result
      $this->output($json);
    }
