...
class WPAPP_Introspector {
  ...
  public function get_current_category() {
    global $wpapp;
    extract($wpapp->query->get(array('id', 'slug', 'category_id', 'category_slug')));
    if ($id || $category_id) {
      if (!$id) {
        $id = $category_id;
      }
  ...
  public function get_current_tag() {
    global $wpapp;
    extract($wpapp->query->get(array('id', 'slug', 'tag_id', 'tag_slug')));
    if ($id || $tag_id) {
      if (!$id) {
        $id = $tag_id;
      }
  ...
  public function get_current_author() {
    global $wpapp;
    extract($wpapp->query->get(array('id', 'slug', 'author_id', 'author_slug')));
    if ($id || $author_id) {
      if (!$id) {
        $id = $author_id;
      }
  ...
  public function get_comments($post_id) {
    ...
    foreach ($wp_comments as $wp_comment) {
      $comments[] = new WPAPP_Comment($wp_comment);
    }
    return $comments;
  }
  ...
  public function get_attachments($post_id) {
    ...
    if (!empty($wp_attachments)) {
      foreach ($wp_attachments as $wp_attachment) {
        $attachments[] = new WPAPP_Attachment($wp_attachment);
      }
    }
    return $attachments;
  }
  ...
  public function attach_child_posts(&$post) {
    ...
    foreach ($wp_children as $wp_post) {
      $post->children[] = new WPAPP_Post($wp_post);
    }
  ...
  protected function get_category_object($wp_category) {
    ...
    return new WPAPP_Category($wp_category);
  }
  ...
  protected function set_posts_query($query = false) {
    ...
    if ($wpapp->query->page) {
      $query['paged'] = $wpapp->query->page;
    }
    
    if ($wpapp->query->count) {
      $query['posts_per_page'] = $wpapp->query->count;
    }
    
    if ($wpapp->query->post_type) {
      $query['post_type'] = $wpapp->query->post_type;
    }
