...
class WPAPP_Query {
  ...
  function get_legacy_controller($json) {
    ...
    if ($json == 'submit_comment') {
      if ($wpapp->controller_is_active('respond')) {
        return 'respond';
      } else {
        $wpapp->error("The 'submit_comment' method has been removed from the Core controller. To use this method you must enable the Respond controller from WP Admin > Settings > WPAPP.");
      }
    } else if ($json == 'create_post') {
      if ($wpapp->controller_is_active('posts')) {
        return 'posts';
      } else {
        $wpapp->error("The 'create_post' method has been removed from the Core controller. To use this method you must enable the Posts controller from WP Admin > Settings > WPAPP.");
      }
    } else {
      return 'core';
    }
  ...
  function get_method($controller) {
    ...
    if (empty($method)) {
      ...
      return false;
    } else if (method_exists("WPAPP_{$controller}_Controller", $method)) {
      ...
      if (is_search()) {
        ...
        if (empty($_GET['json'])) {
          $wpapp->error("Uknown method '$method'.");
        }
